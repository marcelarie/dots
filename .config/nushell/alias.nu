...
alias nurc = do { cd $nu_config; nvim config.nu } # Edit nu config
alias nuenv = do { cd $nu_config; nvim env.nu } # Edit nu env
alias nual = do { cd $nu_config; nvim alias.nu } # Edit nu alias
alias l = ls
alias ll = ls -l
alias lla = ls -la
alias el = ^eza
...
alias skrc = ^nvim ~/.config/sketchybar/sketchybarrc
...
alias ts = ^tmux switch -t (tmux ls |  awk '{print $1}' | sed s/:// | fzy)
alias tks = ^tmux kill-session -t (^tmux ls |  awk '{print $1}' | sed s/:// | fzy)
alias tmsc = ^tmux switch-client -t
# alias two = run-external $"($env.HOME)/scripts/tmux/create-session.sh" "work"
# alias tpe = run-external $"($env.HOME)/scripts/tmux/create-session.sh" "own"
